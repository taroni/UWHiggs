...
files=    glob.glob('results/%s/test_pk_njet_mm/*.root' % (jobid))
...
sign = ['os']
miso = ['mLoose']
outputdir = 'plots/%s/test_pk_njet_mm/' % (jobid)
...
SMH = views.StyleView(views.SumView( *[ plotter.get_view(regex) for regex in filter(lambda x : 'HToTauTau' in x , mc_samples)]), **remove_name_entry(data_styles['SMH*']))
...
histoname = [('Njets','num_jets',1), ('ZMass', 'm1-m2 Inv Mass (GeV)',1)]
