...
class LFVHEMuAnalyzerMVA800(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVA800, self).__init__(tree, outfile, **kwargs)
...
#        alldirs=['','allfakes/','allfakesUp/','allfakesDown/','allfakes_nofrweight/','subtracted/','subtractedup/','subtracteddown/','mLoose_nofrweight/','mLoose/','mLooseUp/','mLooseDown/','eLoose_nofrweight/','eLoose/','eLooseUp/','eLooseDown/','mLooseeLoose_nofrweight/','mLooseeLoose/','mLooseeLooseUp/','mLooseeLooseDown/']
 ...
 #                   for k in threshold:
    ...
    def fill_histos(self, row, f=None,region=None,btagSf=1,sys=''):
        if self.is_WJet or self.is_DYJet:
 ...
 #               nbtagged=2
  #          btagweight=1
   #         if (self.isData and nbtagged>0):
    #            continue
     #       elif nbtagged>0:
      #          btagweight=bTagSF.bTagEventWeight(nbtagged,row.jb1pt,row.jb1flavor,row.jb2pt,row.jb2flavor,1,0,0)
