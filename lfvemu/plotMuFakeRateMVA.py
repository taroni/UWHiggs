...
files=    glob.glob('results/%s/MuFakeRateAnalyzerMVA/*.root' % (jobid))
...
sign = ['os']
muiso = ['mLoose', 'mTight']
outputdir = 'plots/%s/MuFakeRateAnalyzerMVA/' % (jobid)
...
new_mc_samples.extend(['EWKDiboson', 'DYLL'])

print new_mc_samples

histoname = [('mPt','#mu p_{T} (GeV)',2),('mAbsEta','#mu  #abseta',4),('mEta','#mu #eta',2), ('ZMass', '#mu-#mu Inv Mass (GeV)',1)]
