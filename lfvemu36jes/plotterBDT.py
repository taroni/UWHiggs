...
#new_mc_samples.extend(['WGamma','SingleT','EWKDiboson','SMH','TT','Wplus','QCD','DYLL'])
...
##      ('mPFMETDeltaPhi_vs_ePFMETDeltaPhi','mPFMETDeltaPhi_vs_ePFMETDeltaPhi',1)
   ...
   if not os.path.exists(outputdir+"/os"):
         os.makedirs(outputdir+"/os")
...
#      plotter.set_subdir(os.path.join('embedded',path)) if embedded else plotter.set_subdir(path)
      ...
      for var, xlabel, rebin in histo_info:
         ...
         if "BDT" not in var:continue
         print var
         if (int(njet)==1 ):
             rebin = rebin*2
         if ( int(njet)==22 or int(njet)==21):
             rebin=rebin*2
