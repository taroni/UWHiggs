...
class LFVHEMuAnalyzerMVApostBDTcollmass(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVApostBDTcollmass, self).__init__(tree, outfile, **kwargs)
...
#        self.var_d_star =['mTmuMet_','mTeMet_','deltaPhimue_','eDphiPFMet_','mDphiPFMet_','metEt_','numjets_','vbfMass_','vbfDeltaEta_','deltaeta_mu_e:=mEta_-eEta_'] 
        self.var_d_star =['mTmuMet_','mTeMet_','eDphiPFMet_','mDphiPFMet_','mColl_'] 
        self.xml_name = os.path.join(os.getcwd(),"BDTcollmass/weights/TMVAClassification_BDT.weights.xml")  #weights from BDT
 ...
 #                   for k in threshold:
        ...
        for k in range(len(folder)):
            ...
            if k<515:
                ...
                self.book(f, "BDT_output", "BDT_output",56,-0.9,0.5)
...
#                print "btagweight,nbtagged,row.jb1pt,row.jb1hadronflavor,row.jb2pt,row.jb2hadronflavor"," ",btagweight," ",nbtagged," ",row.jb1pt," ",row.jb1hadronflavor," ",row.jb2pt," ",row.jb2hadronflavor
            ...
            for sys in self.sysdir:
                ...
                self.var_d_0 ={'mTmuMet_':shifted_mMtToPfMet,'mTeMet_':shifted_eMtToPfMet,'eDphiPFMet_':abs(shifted_eDPhiToPfMet),'mDphiPFMet_':abs(shifted_mDPhiToPfMet),'mColl_':collmass(row,shifted_type1_pfMetEt,shifted_type1_pfMetPhi)}
                ...
                self.fill_histos(row,MVA0,folder,region,btagweight,sys)
