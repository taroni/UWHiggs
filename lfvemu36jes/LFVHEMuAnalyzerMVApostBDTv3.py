...
class LFVHEMuAnalyzerMVApostBDTv2(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVApostBDTv2, self).__init__(tree, outfile, **kwargs)
...
#        self.var_d_star =['mTmuMet_','mTeMet_','deltaPhimue_','eDphiPFMet_','mDphiPFMet_','metEt_','numjets_','vbfMass_','vbfDeltaEta_','deltaeta_mu_e:=mEta_-eEta_'] 
        self.var_d_star =['mTmuMet_','mTeMet_','eDphiPFMet_','mDphiPFMet_','lepAsym_'] 
        self.xml_name = os.path.join(os.getcwd(),"BDTlepDY/weights/TMVAClassification_BDT.weights.xml")  #weights from BDT
...
#                print "btagweight,nbtagged,row.jb1pt,row.jb1hadronflavor,row.jb2pt,row.jb2hadronflavor"," ",btagweight," ",nbtagged," ",row.jb1pt," ",row.jb1hadronflavor," ",row.jb2pt," ",row.jb2hadronflavor
            ...
            self.var_d_00 ={'mTmuMet_':row.mMtToPfMet_type1,'mTeMet_':row.eMtToPfMet_type1,'eDphiPFMet_':abs(row.eDPhiToPfMet_type1),'mDphiPFMet_':abs(row.mDPhiToPfMet_type1),'lepAsym_':leptonAsymmetry}
            ...
            for sys in self.sysdir:
                ...
                self.var_d_0 ={'mTmuMet_':shifted_mMtToPfMet,'mTeMet_':shifted_eMtToPfMet,'eDphiPFMet_':abs(shifted_eDPhiToPfMet),'mDphiPFMet_':abs(shifted_mDPhiToPfMet),'lepAsym_':leptonAsymmetry}
                ...
                if jn==0:
                    if MVA0<0.02:continue
                if jn==1:
                    if MVA0<0.16:continue
                if jn==21:
                    if MVA0<-0.01:continue
                if jn==22:
                    if MVA0<0.07:continue
