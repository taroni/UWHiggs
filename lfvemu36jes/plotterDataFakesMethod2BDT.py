...
Fakes = views.StyleView(
    views.SumView( 
        *[ plotter.get_view(regex) for regex in 
          filter(lambda x : x.startswith('FAKESmethod2') , mc_samples )]
...
new_mc_samples.extend(['WGamma','SMH','Fakes','EWKDiboson','SingleT','TT','DYLL','ZTT'])
...
if not no_plots:
   ...
   for regions in ['fakeRateMethod2']:
...
#          plotter.set_subdir(os.path.join('embedded',path)) if embedded else plotter.set_subdir(path)
           if not os.path.exists(outputdir+'fakeRateMethod2/'+path):
               os.makedirs(outputdir+'fakeRateMethod2/'+path)
               os.makedirs(outputdir+'fakeRateMethod2/'+path+'/selected')
#          if int(njet)==2:
...
#             plotter.save(path+"/vbfDeta",dotroot=True)
 ...
 #              print var
               ...
               print "**************************************************************************************************************************************************"
               plotter.save('fakeRateMethod2/'+path+"/"+njet+"_preselection_"+var,dotroot=True)
...
#               print var
               ...
               plotter.save('fakeRateMethod2/'+path+"/selected/"+njet+"_selection_"+var,dotroot=True)
