...
mc_samples = [
    'WW*', 'ZZ*','WZ*', 'VV*',
    'GluGluHToTauTau_M125*', 'ttHJet*', 'ST_*', 'TT_*', 'VBFHToTauTau_M125*', 'WG*',   'WminusHToTauTau_M125*', 'WplusHToTauTau_M125*', 'ZHToTauTau_M125*',#'EWK*', 
    'WJetsToLNu*','W1JetsToLNu*','W2JetsToLNu*','W3JetsToLNu*','W4JetsToLNu*','DYJetsToLL_M-10to50*', 'DY1JetsToLL_M-10to50*', 'DY2JetsToLL_M-10to50*', 
...
for x in mc_samples:
    files.extend(glob.glob('results/%s/EMAnalyzer/%s.root' % (jobid, x)))
    lumifiles.extend(glob.glob('inputs/%s/%s.lumicalc.sum' % (jobid, x)))

print lumifiles

outputdir = 'plots/%s/EMAnalyzer/%s/' % (jobid, channel)
...
histoname = [('ePt', 'e p_{T} (GeV)', pt_vars_binning, False ),
             ('ePhi','e #phi',2, False ) ,('eEta','e #eta',1, False),
             ('mPt','#mu p_{T} (GeV)',pt_vars_binning, False) ,
             ('mPhi','#mu #phi',1, False), ('mEta','#mu #eta',1, False),
             ('e_m_DPhi','#Delta#phi(e,#mu)',1, True), ('e_m_DR','#DeltaR(e,#mu)',1, True), ('e_m_Mass', 'e-#mu Visible Mass (GeV)',col_vis_mass_binning, False),
             ('type1_pfMetEt', 'type1PF MET (GeV)', met_vars_binning, False) ,
...
#             ('ePFMET_DeltaPhi','#Delta#phi(e, MET)', 1, True),
             ...
             ('mMtToPfMet_type1','M_{T}(#mu, MET) (GeV)',5, False),
             ('e_m_PZeta', 'e_m_PZeta', 2, True), ('e_m_PZetaLess0p85PZetaVis', 'e_m_PZetaLess0p85PZetaVis', 2, True), ('e_m_PZetaVis', 'e_m_PZetaVis', 2, False),
...
*[ plotter.get_view(regex) for regex in \
   filter(lambda x : x.startswith('WW') or x.startswith('WZ') or x.startswith('ZZ') or x.startswith('WG') or x.startswith('VV') , mc_samples )]), **remove_name_entry(data_styles['WW*']))
...
new_mc_samples = ['DYTT','EWKDiboson','Wjets', 'TT', 'ST', 'SMH', 'DY']
...
for foldername in foldernames:
    ...
    for n,h in enumerate(histoname) :
        plotter.pad.SetLogy()
        if 'DeltaPhi' in h[0] or 'ePhi' in h[0] or 'eEta' in h[0] or 'mPhi' in h[0] or 'mEta' in h[0]: plotter.pad.SetLogy(False)
        #print foldername
