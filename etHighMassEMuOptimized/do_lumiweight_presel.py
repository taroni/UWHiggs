...
if args.numCategories==3:
   category_names=["etaumu_0jet_presel","etaumu_1jet_presel","etaumu_2jet_presel"]
elif args.numCategories==2:
   category_names=["etaumu_01jet_presel","etaumu_rest_presel"]
else:
   print "number of categories must be 1 or 2"
   exit

if not os.path.exists(args.outputdir+"/Simple"+args.analyzer_name+str(args.Lumi)+"/preselection"):
   os.makedirs(args.outputdir+"/Simple"+args.analyzer_name+str(args.Lumi)+"/preselection")
if not os.path.exists(args.outputdir+"/Simple"+args.analyzer_name+str(args.Lumi)+"/preselection/"+args.region):
   os.makedirs(args.outputdir+"/Simple"+args.analyzer_name+str(args.Lumi)+"/preselection/"+args.region)
...
#            print histo.GetBinContent(bin),bin
   ...
   outputfile=ROOT.TFile(args.outputdir+"/Simple"+args.analyzer_name+str(args.Lumi)+"/preselection/"+args.region+"/"+var[0]+".root","recreate")
