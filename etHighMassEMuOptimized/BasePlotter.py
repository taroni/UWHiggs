...
class BasePlotter(Plotter):
    def __init__ (self, blind_region=None, forceLumi=-1, use_embedded=False): 
        ...
        self.files     = glob.glob('results/%s/EMAnalyzer/*.root' % jobid)
        self.lumifiles = glob.glob('inputs/%s/*.lumicalc.sum' % jobid)

        self.outputdir = 'plots/%s/EMAnalyzer' % jobid
        ...
        self.mc_samples = [
            'WW*', 'ZZ*','WZ*', 'VV*', 'GluGluHToTauTau_M125*', 'ttHJet*', 'ST_*', 'TT_*', 'VBFHToTauTau_M125*', 'WG*',   'WminusHToTauTau_M125*', 'WplusHToTauTau_M125*', 'ZHToTauTau_M125*',#'EWK*', 
        ...
        self.views['qcd'] = {'view' : self.make_qcd()}
        self.datacard_names = {
           # '*HTo[WT][WT]'           : 'SMH126'   ,
            'SMH'                    : 'SMH',
            'TT'                     : 'ttbar'     ,
    ...
    def make_qcd(self, ):
        ...
        qcd = views.SubdirectoryView(data_view, 'QCD')
        mc_views = self.mc_views()
        if self.use_embedded:            
            mc_views.append(self.get_view('ZetauEmbedded'))
        mc_sum = views.SumView(*mc_views)
        mc_sum_t = views.SubdirectoryView(mc_sum, 'QCD')
        qcd_view = SubtractionView(qcd, mc_sum_t, restrict_positive=True)
        style = data_styles['QCD*']
    ...
    def plot_with_bkg_uncert (self, folder, variable, rebin=1, xaxis='',
        ...
        #get fakes
        qcd_view = self.get_view('qcd')
        if preprocess:
            qcd_view = preprocess(qcd_view)
        qcd_view = RebinView(qcd_view, rebin)
        qcd = qcd_view.Get(path)
        qcd = SystematicsView.add_error(qcd, 0.20) 
        ...
        mc_stack_view =views.StackView(*mc_views, sorted=True) 
        mc_stack = mc_stack_view.Get(path)
            ...
            if self.blind_region:
                for bin in range(data.GetNbinsX()+1):
                    ...
                    for histo in sig:
                        ...
                        if bool(bg_count==0 and sig_count>0) or ((float(sig_count)/float(sig_count+bg_count))>0.1):
