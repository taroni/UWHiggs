...
#p2 muon tigger uncertently is for single muon trigger ,run under 257819
...
# p3 muon tigger uncerntainty for run after 257819
...
#single muon trigger combined with lumi weight, with the fomular (p2*(16.344578402+ 377.733675446) + P3*1716.3544157) /(1716.3544157+377.733675446+16.344578402)
def correct_mutrig15_mva_combined(pt,abseta):
    if abseta< 0.9:
        if pt <25.0:
           return 0.987 
 	elif pt < 30:
	   ...
	   return 1.015
def correct_mutrig15_p1s_mva_combined(pt,abseta):
    if abseta< 0.9:
        if pt <25.0:
           return 0.992 
 	elif pt < 30:
	   ...
	   return 1.024
def correct_mutrig15_m1s_mva_combined(pt,abseta):
    if abseta< 0.9:
        if pt <25.0:
           return 0.983 
 	elif pt < 30:
