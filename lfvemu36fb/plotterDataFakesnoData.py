...
#print "files",files
outputdir = 'plotsNoData/'+sys.argv[3]+'/'+Analyzer+'/' 

plotter = BasePlotter(files, outputdir, blind_region,use_embedded=embedded,blind_path="fakeRateMethod/os/.*ass*")
...
new_mc_samples.extend(['SMH','WGamma','SingleT','EWKDiboson','TT','Fakes','DYLL'])
...
#               if int(njet)==22:
 #                  rebin=rebin*2
               ...
               plotter.plot_mc_vs_data_noData(path, var, njet,rebin, xlabel,
                                                    leftside=False, xrange=(-4.0,300.), show_ratio=False, ratio_range=1., 
                                                    sort=True,drawData=False,drawLegend=False,br=50)
...
#               print var
               logging.debug("Plotting %s/%s" % (path, var) )
               plotter.pad.SetLogy(False)
               plotter.plot_mc_vs_data_noData(path+'/selected/nosys', var, njet,rebin, xlabel,
                                                 leftside=False, xrange=(-4.0,300.), show_ratio=False, ratio_range=1., 
                                          sort=True,drawData=False,drawLegend=False,br=5)
