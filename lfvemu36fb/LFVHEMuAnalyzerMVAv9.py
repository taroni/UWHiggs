...
class LFVHEMuAnalyzerMVAv9(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVAv9, self).__init__(tree, outfile, **kwargs)
...
#            print "non-repeat"
            ...
            btagweight=1
            if nbtagged>2 or nbtagged==0:
                continue
            if (self.isData and nbtagged!=1):
                continue

            if not self.isData and nbtagged==1: 
...
#            if not self.isData and nbtagged==2:
#                btagweight=bTagSF.bTagEventWeight(nbtagged,row.jb1pt,row.jb1hadronflavor,row.jb2pt,row.jb2hadronflavor,1,0,1) if (row.jb1pt>-990 and row.jb1hadronflavor>-990 and row.jb2pt>-990 and row.jb2hadronflavor>-990) else 0
