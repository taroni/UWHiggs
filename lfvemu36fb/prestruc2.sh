...
python getQCD2.py $analyzer $luminosity $jobid
...
python getFakesmethod2.py $analyzer $luminosity $jobid   #get the same without sysuncertainty in histos, combine script already adds them
...
cp FAKESmethod2forcombine.root LFVHEMuAnalyzerMVA$analyzer$luminosity'fakesfromdata'/FAKESmethod2.root
