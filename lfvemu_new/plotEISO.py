...
embedded = False
print jobid
files=  glob.glob('results/%s/LFVHEMuAnalyzerMVA_remveto/*.root' % (jobid))
#print "files",files
outputdir = 'plots/%s/lfvemu/LFVHEMuAnalyzerMVA_remveto/' % (jobid)
...
#      plotter.set_subdir(os.path.join('embedded',path)) if embedded else plotter.set_subdir(path)
      ...
      for var, xlabel, rebin in histo_info:
         ...
         plotter.plot_mc_vs_data_witherrors(path, var, njet,rebin, xlabel,
                                                leftside=False, xrange=(0.0,2.0), show_ratio=True, ratio_range=1., 
                                                sort=True,br=20)
