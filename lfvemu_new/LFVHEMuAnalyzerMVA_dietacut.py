...
class LFVHEMuAnalyzerMVA_dietacut(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVA_dietacut, self).__init__(tree, outfile, **kwargs)
...
#                print "btagweight,nbtagged,row.jb1pt,row.jb1flavor,row.jb2pt,row.jb2flavor"," ",btagweight," ",nbtagged," ",row.jb1pt," ",row.jb1flavor," ",row.jb2pt," ",row.jb2flavor
            ...
            for sys in self.sysdir:
                ...
                if jn > 3 : jn = 3
                if jn==2:
                    if shifted_vbfMass>=550 and shifted_vbfDeta>=3.5:
                        jn=22
                    else:
                        jn=21
                if jn == 0 :
