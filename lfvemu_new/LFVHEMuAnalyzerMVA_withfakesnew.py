...
class LFVHEMuAnalyzerMVA_withfakesnew(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVA_withfakesnew, self).__init__(tree, outfile, **kwargs)
...
#        pudir = ['','p1s/', 'm1s/','trp1s/', 'trm1s/', 'eidp1s/','eidm1s/',  'eisop1s/','eisom1s/', 'mLoose/','mLooseUp/','mLooseDown/', ]
        alldirs=['','allfakes/','subtracted/','allfakes_nofrweight/']#,'allfakesUp/','allfakesDown/','allfakes_nofrweight/','subtracted/','subtractedup/','subtracteddown/','mLoose_nofrweight/','mLoose/','mLooseUp/','mLooseDown/','eLoose_nofrweight/','eLoose/','eLooseUp/','eLooseDown/','mLooseeLoose_nofrweight/','mLooseeLoose/','mLooseeLooseUp/','mLooseeLooseDown/']
#        alldirs=['']
        for d  in alldirs :
            for i in sign:
...
#        emlooseList= ['mLooseeLoose_nofrweight/','mLooseeLoose/','mLooseeLooseUp/','mLooseeLooseDown/']
        alllooselist=['allfakes_nofrweight/','allfakes/']
...
#      fr_weights = [1,efr_weights[1]*mfr_weights[1],efr_weights[1]*mfr_weights[1]+comb_error,efr_weights[1]*mfr_weights[1]-comb_error]
        if region=='eLoosemTight':
            ...
            for n, l in enumerate(alllooselist) :
                frweight = [weight,weight*fakerateWeight]
                folder = l+f
                if sys=='presel' or sys=='nosys':
                    histos[folder+'/h_collmass_pfmet'].Fill(collmass(row,row.type1_pfMetEt, row.type1_pfMetPhi),frweight[n])
                    histos[folder+'/mPt'].Fill(row.mPt, frweight[n])
                    histos[folder+'/mEta'].Fill(row.mEta, frweight[n])
                    histos[folder+'/mPhi'].Fill(row.mPhi, frweight[n]) 
                    histos[folder+'/ePt'].Fill(row.ePt, frweight[n])
                    histos[folder+'/eEta'].Fill(row.eEta, frweight[n])
                    histos[folder+'/ePhi'].Fill(row.ePhi, frweight[n])
                    histos[folder+'/em_DeltaPhi'].Fill(deltaPhi(row.ePhi, row.mPhi), frweight[n])
                    histos[folder+'/em_DeltaR'].Fill(row.e_m_DR, frweight[n])
                    histos[folder+'/h_vismass'].Fill(row.e_m_Mass, frweight[n])
                    histos[folder+'/ePFMET_Mt'].Fill(row.eMtToPfMet_type1, frweight[n])
                    histos[folder+'/mPFMET_Mt'].Fill(row.mMtToPfMet_type1, frweight[n])
                    histos[folder+'/ePFMET_DeltaPhi'].Fill(abs(row.eDPhiToPfMet_type1), frweight[n])
                    histos[folder+'/mPFMET_DeltaPhi'].Fill(abs(row.mDPhiToPfMet_type1), frweight[n])
 #                   histos[folder+'/mPFMETDeltaPhi_vs_ePFMETDeltaPhi'].Fill(abs(row.mDPhiToPfMet_type1),abs(row.eDPhiToPfMet_type1) , frweight[n])
                    histos[folder+'/vbfMass'].Fill(row.vbfMass, frweight[n])
                    histos[folder+'/vbfDeta'].Fill(row.vbfDeta, frweight[n])
                    histos[folder+'/jetN_30'].Fill(row.jetVeto30, frweight[n]) 
                elif sys=='jetup':
                    histos[folder+'/h_collmass_pfmet'].Fill(collmass(row, row.type1_pfMet_shiftedPt_JetEnUp, row.type1_pfMet_shiftedPhi_JetEnUp),frweight[n])
                elif sys=='jetdown':
                    histos[folder+'/h_collmass_pfmet'].Fill(collmass(row, row.type1_pfMet_shiftedPt_JetEnDown, row.type1_pfMet_shiftedPhi_JetEnDown),frweight[n])
                elif sys=='tup':
                    histos[folder+'/h_collmass_pfmet'].Fill(collmass(row, row.type1_pfMet_shiftedPt_TauEnUp, row.type1_pfMet_shiftedPhi_TauEnUp),frweight[n])
                elif sys=='tdown':
                    histos[folder+'/h_collmass_pfmet'].Fill(collmass(row, row.type1_pfMet_shiftedPt_TauEnDown, row.type1_pfMet_shiftedPhi_TauEnDown),frweight[n])
                elif sys=='uup':
                    histos[folder+'/h_collmass_pfmet'].Fill(collmass(row, row.type1_pfMet_shiftedPt_UnclusteredEnUp, row.type1_pfMet_shiftedPhi_UnclusteredEnUp),frweight[n])
                elif sys=='udown':
                    histos[folder+'/h_collmass_pfmet'].Fill(collmass(row, row.type1_pfMet_shiftedPt_UnclusteredEnDown, row.type1_pfMet_shiftedPhi_UnclusteredEnDown),frweight[n])
                    """
        if region=='eTightmLoose' and (sys=='presel' or sys=='nosys'):
            ...
            if not self.isData:
                subfolder=pudir[1]+f
                subweight=fakerateWeight*weight*(-1)
                histos[subfolder+'/h_collmass_pfmet'].Fill(collmass(row, row.type1_pfMetEt, row.type1_pfMetPhi),subweight)
                histos[subfolder+'/mPt'].Fill(row.mPt, subweight)
                histos[subfolder+'/mEta'].Fill(row.mEta, subweight)
                histos[subfolder+'/mPhi'].Fill(row.mPhi, subweight) 
                histos[subfolder+'/ePt'].Fill(row.ePt, subweight)
...
#            if btagweight==0: continue
            ...
            if self.is_WJet or self.is_DYJet:
                self.fill_jet_histos(row,sign,region)
