...
class LFVHEMuAnalyzerMVA_NUP(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVA_NUP, self).__init__(tree, outfile, **kwargs)
...
#                self.book(f, "jetN_30", "Number of jets, p_{T}>30", 10, -0.5, 9.5) 
    ...
    def fill_histos(self, row, f=None,region=None,btagweight=1,sys=''):
        if self.is_WJet or self.is_DYJet:
            weight = self.event_weight(row)*self.binned_weight[int(row.NUP)-5]*0.001
