...
class LFVHEMuAnalyzerMVApostBDTv4(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVApostBDTv4, self).__init__(tree, outfile, **kwargs)
...
#        self.var_d_star =['mTmuMet_','mTeMet_','deltaPhimue_','eDphiPFMet_','mDphiPFMet_','metEt_','numjets_','vbfMass_','vbfDeltaEta_','deltaeta_mu_e:=mEta_-eEta_'] 
        ...
        self.xml_name = os.path.join(os.getcwd(),"BDTlep/weights/TMVAClassification_BDT.weights.xml")  #weights from BDT
...
#                print "btagweight,nbtagged,row.jb1pt,row.jb1hadronflavor,row.jb2pt,row.jb2hadronflavor"," ",btagweight," ",nbtagged," ",row.jb1pt," ",row.jb1hadronflavor," ",row.jb2pt," ",row.jb2hadronflavor
            ...
            self.var_d_00 ={'mTmuMet_':row.mMtToPfMet_type1,'mTeMet_':row.eMtToPfMet_type1,'deltaPhimue_':abs(row.e_m_DPhi),'eDphiPFMet_':abs(row.eDPhiToPfMet_type1),'mDphiPFMet_':abs(row.mDPhiToPfMet_type1),'lepAsym_':leptonAsymmetry}
