...
class LFVHEMuAnalyzerMVA_fakerate(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVA_fakerate, self).__init__(tree, outfile, **kwargs)
...
#        alldirs=['','allfakes/','allfakesUp/','allfakesDown/','allfakes_nofrweight/','subtracted/','subtractedup/','subtracteddown/','mLoose_nofrweight/','mLoose/','mLooseUp/','mLooseDown/','eLoose_nofrweight/','eLoose/','eLooseUp/','eLooseDown/','mLooseeLoose_nofrweight/','mLooseeLoose/','mLooseeLooseUp/','mLooseeLooseDown/']
        alldirs=['','allfakes/','allfakesUp/','allfakesDown/','allfakes_nofrweight/','subtracted/','subtractedup/','subtracteddown/']
        for d  in alldirs :
            for i in sign:
...
#        if region!='signal':
        ...
        if region=='eLoosemTight':
            if (row.ePt>0 and row.ePt<=40):
                fakerateFactor=0.6
            elif (row.ePt>40 and row.ePt<=50):
                fakerateFactor=0.68
            elif (row.ePt>50 and row.ePt<=70):
                fakerateFactor=0.723
            elif (row.ePt>70 and row.ePt<=100):
                fakerateFactor=0.8
            elif row.ePt>100:
                fakerateFactor=0.875
            fakerate_weight=fakerateFactor/(1-fakerateFactor)
