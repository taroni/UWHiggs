...
class LFVHEMuAnalyzerMVABDTmassFitv1(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVABDTmassFitv1, self).__init__(tree, outfile, **kwargs)
...
#                print "btagweight,nbtagged,row.jb1pt,row.jb1hadronflavor,row.jb2pt,row.jb2hadronflavor"," ",btagweight," ",nbtagged," ",row.jb1pt," ",row.jb1hadronflavor," ",row.jb2pt," ",row.jb2hadronflavor
            ...
            for sys in self.sysdir:
                ...
                MVAval=MVAcutvalue
                if jn==0:
                    if MVAval<0.24:continue
                if jn==1:
                    if MVAval<0.17:continue
                if jn==21:
                    if MVAval<-0.06:continue
                if jn==22:
                    if MVAval<0.06:continue
