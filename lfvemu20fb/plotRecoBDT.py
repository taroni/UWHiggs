...
embedded = False
print jobid
files=  glob.glob('results/%s/LFVHEMuAnalyzerMVA_postBDTwithpt/*.root' % (jobid))
#print "files",files
outputdir = 'plots/%s/lfvemu/LFVHEMuAnalyzerMVA_postBDTwithpt/' % (jobid)
...
new_mc_samples.extend(['DYLL', 'Wplus','TT','EWKDiboson','SMH','SingleT'])
...
#      plotter.set_subdir(os.path.join('embedded',path)) if embedded else plotter.set_subdir(path)
      ...
      for var, xlabel, rebin in histo_info:
         ...
         print var
         if (int(njet)==2 or int(njet)==1):
             rebin = rebin*2
         if ( int(njet)==22):
             rebin=rebin*2
         plotter.plot_mc_vs_data_witherrors(path, var, njet,rebin, xlabel,
                                                leftside=False, xrange=(-0.6,0.6), show_ratio=True, ratio_range=1., 
                                                sort=True,br=20)
