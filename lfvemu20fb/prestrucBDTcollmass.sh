...
python getQCDforcombineBDTcoll.py $analyzer $luminosity $jobid   #get the same without sysuncertainty in histos, combine script already adds them
...
cp QCDforcombineBDTcoll.root LFVHEMuAnalyzerMVA$analyzer$luminosity'fakesfromMC'/QCD.root
