...
class LFVHEMuAnalyzerMVABDTmassFit(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVABDTmassFit, self).__init__(tree, outfile, **kwargs)
        target = os.path.basename(os.environ['megatarget'])
        self.target=target
        self.var =['mTmuMet_','mTeMet_','deltaPhimue_','eDphiPFMet_','mDphiPFMet_','pZeta_'] 
        self.xml_name = os.path.join(os.getcwd(),"BDT_TTonly_fit_on_Mcoll3/weights/TMVAClassification_BDT.weights.xml")  #weights from BDT
...
#                print "btagweight,nbtagged,row.jb1pt,row.jb1hadronflavor,row.jb2pt,row.jb2hadronflavor"," ",btagweight," ",nbtagged," ",row.jb1pt," ",row.jb1hadronflavor," ",row.jb2pt," ",row.jb2hadronflavor
            ...
            self.var_2jet ={'mTmuMet_':row.mMtToPfMet_type1,'mTeMet_':row.eMtToPfMet_type1,'deltaPhimue_':abs(row.e_m_DPhi),'eDphiPFMet_':abs(row.eDPhiToPfMet_type1),'mDphiPFMet_':abs(row.mDPhiToPfMet_type1),'pZeta_':row.e_m_PZeta}
                
            MVA2jet=self.functor(**self.var_2jet)

            MVAval=MVA2jet

            if MVAval<-0.4996333054 and MVAval>-0.49963330547:
                print MVAval,'    mTmuMet  ',row.mMtToPfMet_type1,'    mTeMet  ',row.eMtToPfMet_type1,'    deltaPhimue  ',row.e_m_DPhi,'    eDphiPFMet  ',row.eDPhiToPfMet_type1,'    mDphiPFMet  ',row.mDPhiToPfMet_type1,'   pZeta_ ',row.e_m_PZeta
            ...
            for sys in self.sysdir:
                ...
                if jn==2:
                    if row.vbfMass>=550:
                        ...
                        jn=21
                self.var_d ={'mTmuMet_':shifted_mMtToPfMet,'mTeMet_':shifted_eMtToPfMet,'deltaPhimue_':abs(row.e_m_DPhi),'eDphiPFMet_':abs(shifted_eDPhiToPfMet),'mDphiPFMet_':abs(shifted_mDPhiToPfMet),'pZeta_':row.e_m_PZeta}
                ...
                if jn==1:
                    if MVAval<0.05:continue
                if jn==21:
                    if MVAval<-0.04:continue
                if jn==22:
                    if MVAval<0.05:continue
