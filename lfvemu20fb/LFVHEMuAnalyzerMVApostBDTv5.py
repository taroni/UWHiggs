...
class LFVHEMuAnalyzerMVApostBDTv5(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVApostBDTv5, self).__init__(tree, outfile, **kwargs)
        target = os.path.basename(os.environ['megatarget'])
        self.target=target

        self.var_d_01jet =['mTmuMet_','mTeMet_','eDphiPFMet_','mDphiPFMet_','lepAsym_'] 
        self.xml_name_01jet = os.path.join(os.getcwd(),"BDT01jet/weights/TMVAClassification_BDT.weights.xml")  #weights from BDT
        self.functor_01jet = FunctorFromMVA('BDT method',self.xml_name_01jet, *self.var_d_01jet)

        self.var_d_2jet =['mTmuMet_','mTeMet_','deltaPhimue_','eDphiPFMet_','mDphiPFMet_','lepAsym_'] 
        self.xml_name_2jet = os.path.join(os.getcwd(),"BDT2jet/weights/TMVAClassification_BDT.weights.xml")  #weights from BDT
        self.functor_2jet = FunctorFromMVA('BDT method',self.xml_name_2jet, *self.var_d_2jet)
...
#                print "btagweight,nbtagged,row.jb1pt,row.jb1hadronflavor,row.jb2pt,row.jb2hadronflavor"," ",btagweight," ",nbtagged," ",row.jb1pt," ",row.jb1hadronflavor," ",row.jb2pt," ",row.jb2hadronflavor
            ...
            isElecTight=False
            if selections.lepton_id_iso(row, 'e', 'eid15Loose_etauiso0p1'): 
            ...
            self.var_2jet ={'mTmuMet_':row.mMtToPfMet_type1,'mTeMet_':row.eMtToPfMet_type1,'deltaPhimue_':abs(row.e_m_DPhi),'eDphiPFMet_':abs(row.eDPhiToPfMet_type1),'mDphiPFMet_':abs(row.mDPhiToPfMet_type1),'lepAsym_':leptonAsymmetry}
                
            MVA2jet=self.functor_2jet(**self.var_2jet)

            self.var_01jet ={'mTmuMet_':row.mMtToPfMet_type1,'mTeMet_':row.eMtToPfMet_type1,'eDphiPFMet_':abs(row.eDPhiToPfMet_type1),'mDphiPFMet_':abs(row.mDPhiToPfMet_type1),'lepAsym_':leptonAsymmetry}
                
            MVA01jet=self.functor_01jet(**self.var_01jet)
            ...
            for sys in self.sysdir:
                ...
                self.var_2jet ={'mTmuMet_':shifted_mMtToPfMet,'mTeMet_':shifted_eMtToPfMet,'deltaPhimue_':abs(row.e_m_DPhi),'eDphiPFMet_':abs(shifted_eDPhiToPfMet),'mDphiPFMet_':abs(shifted_mDPhiToPfMet),'lepAsym_':leptonAsymmetry}
                
                MVA2jet=self.functor_2jet(**self.var_2jet)

                self.var_01jet ={'mTmuMet_':shifted_mMtToPfMet,'mTeMet_':shifted_eMtToPfMet,'eDphiPFMet_':abs(shifted_eDPhiToPfMet),'mDphiPFMet_':abs(shifted_mDPhiToPfMet),'lepAsym_':leptonAsymmetry}
                ...
                if jn==1:
                    if MVAval<0.1:continue
                if jn==21:
                    if MVAval<0.0:continue
                if jn==22:
                    if MVAval<-0.08:continue
