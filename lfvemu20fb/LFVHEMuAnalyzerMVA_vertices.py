...
class LFVHEMuAnalyzerMVA_vertices(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVA_vertices, self).__init__(tree, outfile, **kwargs)
...
#                    histos[folder+'/mPFMETDeltaPhi_vs_ePFMETDeltaPhi'].Fill(abs(row.mDPhiToPfMet_type1),abs(row.eDPhiToPfMet_type1) , frweight)
        ...
        if region=='eTightmLoose' and (sys=='presel' or sys=='nosys'):
            ...
            if not self.isData and not self.isGluGlu_LFV and not self.isVBF_LFV:
                subfolder=pudir[1]+f
