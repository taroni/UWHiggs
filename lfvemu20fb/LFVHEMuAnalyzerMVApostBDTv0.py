...
class LFVHEMuAnalyzerMVApostBDTv0(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVApostBDTv0, self).__init__(tree, outfile, **kwargs)
        target = os.path.basename(os.environ['megatarget'])
        self.target=target
        self.var =['mTmuMet_','mTeMet_','deltaPhimue_','eDphiPFMet_','mDphiPFMet_','pZeta_'] 
        self.xml_name = os.path.join(os.getcwd(),"BDT_TTonly_fit_on_Mcoll2/weights/TMVAClassification_BDT.weights.xml")  #weights from BDT
