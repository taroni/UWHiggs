...
class LFVHEMuAnalyzerMVA_pieri(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVA_pieri, self).__init__(tree, outfile, **kwargs)
...
#                print "btagweight,nbtagged,row.jb1pt,row.jb1flavor,row.jb2pt,row.jb2flavor"," ",btagweight," ",nbtagged," ",row.jb1pt," ",row.jb1flavor," ",row.jb2pt," ",row.jb2flavor
            ...
            for sys in self.sysdir:
                ...
                if jn > 3 : jn = 3
                if jn==2:
                    if row.vbfMass>=500 and row.vbfDeta>=3.0:
                        jn=22
                    elif row.vbfMass>=150 or row.vbfDeta>1:
                        jn=21
                    else:
                        jn=1
                if jn == 0 :
