...
class ETauAnalyzerWCR(MegaBase):
    tree = 'et/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):
        logging.debug('ETauAnalyzerWCR constructor')
        self.channel='ET'
        super(ETauAnalyzerWCR, self).__init__(tree, outfile, **kwargs)
    ...
    def process(self):
        ...
        for row in self.tree:
            ...
            for sys in sys_directories :
                ...
                totaleEt = self.my_ele.Et() + self.my_MET.Et();
                totalePt = (self.my_ele + self.my_MET).Pt()
                mytMtMet = sqrt(abs(totalEt*totalEt - totalPt*totalPt))
                myeMtMet = sqrt(abs(totalEt*totalEt - totalPt*totalPt))
                ...
                if self.my_ele.Pt() > 60 and self.my_tau.Pt() >30 :
                    ...
                    if (mytMtMet>50 and myeMtMet>50 and myeMtMet<110): ##WCR selection
                    ...
                    if self.my_ele.Pt() > 150 and self.my_tau.Pt()>45:
                        if (mytMtMet>50 and myeMtMet>50 and myeMtMet<110): #WCRselection
