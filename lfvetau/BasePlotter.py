...
class BasePlotter(Plotter):
    def __init__ (self, channel, files, lumifiles, outputdir): 
        cwd = os.getcwd()
...
##
    ...
    def simpleplot_mc(self, folder, variable, rebin=1, xaxis='',
        ...
        self.add_legend(all_var, leftside, entries=len(mc_stack.GetHists()))
    ...
    def get_isoid_unc (self, folder, variable, rebin, preprocess, obj=['e1', 'e2']):
       # 'e1idp1s/','e1idm1s/',  'e1isop1s/','e1isom1s/','e2idp1s/','e2idm1s/',  'e2isop1s/','e2isom1s/'
        ...
        for n, lep in enumerate(obj):
            ...
            #bkg_obj2_iso_m1s_stack_view = self.make_stack(rebin, preprocess, obj2+'isom1s/'+folder, sorted)
            ...
            #bkg_obj2_id_p1s_stack_view = self.make_stack(rebin, preprocess, obj2+'idp1s/'+folder, sorted)
            #bkg_obj2_id_p1s_stack= bkg_obj2_id_p1s_stack_view.Get(variable)
            #histo_obj2_id_p1s =  bkg_obj2_id_p1s_stack.GetStack().Last().Clone()
        
            #bkg_obj2_id_m1s_stack_view = self.make_stack(rebin, preprocess, obj2+'idm1s/'+folder, sorted)
...
#[(histo_obj1_iso_p1s, histo_obj1_iso_m1s), (histo_obj1_id_p1s, histo_obj1_id_m1s), (histo_obj2_iso_p1s, histo_obj2_iso_m1s), (histo_obj2_id_p1s, histo_obj2_id_m1s)]
    ...
    def plot_with_bkg_uncert (self, folder, variable, rebin=1, xaxis='',
        ...
        mc_stack_view = self.make_stack(rebin, preprocess, folder, sorted)
        ...
        if not folder.startswith('tLoose'):
            ...
            xsec_unc_mapper = {
                ...
                't#bar{t}' : 0.026,
                'Single Top' : 0.041,
                'DY (#rightarrow #tau#tau)  + jets' : 0.032, 
        ...
        if 'Met' in variable or 'MET' in variable:
            if not 'mva' in variable or not 'MVA' in variable:
                if 'type1_' in variable or '_Ty1' in variable:
                    ...
                    met_histos.extend([mc_stack_view.Get(newMetName+'_jes'),mc_stack_view.Get(newMetName+'_mes'),mc_stack_view.Get(newMetName+'_tes'),mc_stack_view.Get(newMetName+'_ues')]) ##add the ees
                else:
                    met_histos.extend([mc_stack_view.Get(variable+'_jes'),mc_stack_view.Get(variable+'_mes'),mc_stack_view.Get(variable+'_tes'),mc_stack_view.Get(variable+'_ues')]) ##add the ees
        ...
        while ibin < histo.GetXaxis().GetNbins()+1: 
            ...
            #error on fakes. To put the correct one. Now I'm approximated with a 10%
            diff_fake = abs(histo.GetBinContent(ibin) - finalhisto.GetBinContent(ibin))
            err2 += pow(diff_fake*0.1,2)
        ...
        if show_ratio:
            self.add_ratio_bandplot(data, mc_stack, histo, xrange, ratio_range)
