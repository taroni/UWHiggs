...
sourcepath = 'results/MCntuples_25March/LFVHETauAnalyzer/'
sign = ['os', 'ss']
process = ['gg','vbf']
ptcut = [0, 40]
...
for myfile in lfvfilelist:
        if type(myfile) is list:
                for subfile in myfile :
                        lumifile = 'inputs/MCntuples_25March/'+subfile[:(len(subfile))]+'.lumicalc.sum'
                        f = open(lumifile, 'r')
                ...
                lumifile = 'inputs/MCntuples_25March/'+myfile[:(len(myfile))]+'.lumicalc.sum'
                f = open(lumifile, 'r')
                lumistring = f.readline()
                #print lumistring
                intlumi = float(lumistring)
...
for s in sign :
        ...
        for pr in process:
                ...
                for c in ptcut:
                        #print lineno()
                        filepath = 'plots/MCntuples_25March/LFVHETauAnalyzer/et/'+s+'/'+pr+'/ept'+str(c)
                        ...
                        for nhisto, i  in enumerate(histolist):
                                ...
                                for n, file in enumerate(lfvfilelist):
                                        ...
                                        if type(file) is list :
                                                ...
                                                h0 = file0.Get(s+'/'+pr+'/ept'+str(c)+'/'+i).Clone()
                                                h0.Scale(scalefactors[nfile])
                                                newhisto=h0.Clone()
                                                outfile.cd()
                                                newhistos.append(file0.Get(s+'/'+pr+'/ept'+str(c)+'/'+i).Clone())
                                                ...
                                                for isub, subfile in enumerate(file) :
                                                        ...
                                                        outfile.cd()
                                                        newhistos[n-4].Add(newFile.Get(s+'/'+pr+'/ept'+str(c)+'/'+i).Clone(), scalefactors[nfile])
                                                        #print lineno() , nfile
                                                        newFile.Close()
                                               ...
                                               # LFVStack.Add(histos[nn])
                                                ...
                                                histos.append(ETaufiles[nn].Get(s+'/'+pr+'/ept'+str(c)+'/'+i).Clone())
                                ...
                                LFVStack.Draw('hist')
                                LFVStack.GetHistogram().GetYaxis().SetRangeUser(LFVStack.GetMinimum(), 1.2*LFVStack.GetMaximum() )
                                ...
                                ggLFVHhisto= ggLFVHfile.Get(s+'/'+pr+'/ept'+str(c)+'/'+i).Clone()
                                vbfLFVHhisto= vbfLFVHfile.Get(s+'/'+pr+'/ept'+str(c)+'/'+i).Clone()
